function InitPxVideo(options) {
  ...
  obj.progressBar.addEventListener("click", function(e) {
    ...
    if (!JQuery) {
      ...
      var detailElm = JQuery('.detail-gallery'),
          galleryElm = detailElm.find('.detail-gallery-wrapper');

      offsetLeft = (JQuery(document).width() - galleryElm.width()) / 2;

      if (detailElm.find('.js-fullscreen-controls').length) {
        offsetLeft = 10;
      }
    }

    obj.pos = (e.pageX - offsetLeft) / this.offsetWidth;
  ...
  obj.progressBar.addEventListener("mousemove", function(e) {
    ...
    obj.progressBarHoverContainer.style.left = e.pageX - offsetLeft -25 + "px";
